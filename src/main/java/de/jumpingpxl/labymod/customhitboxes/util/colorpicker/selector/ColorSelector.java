...
public abstract class ColorSelector {
	...
	public abstract void draw(Tessellator tessellator, WorldRenderer buffer);

	public abstract void drawMarker(Tessellator tessellator, WorldRenderer buffer);
	...
	public void renderTextField(Tessellator tessellator, WorldRenderer buffer, int mouseX,
	                            int mouseY) {
		if (Objects.nonNull(textField)) {
	...
	protected GuiTextField createTextField(int width, int height) {
		...
		return new GuiTextField(0, Minecraft.getMinecraft().fontRendererObj, textFieldX, y + 1, width,
				height);
	}
